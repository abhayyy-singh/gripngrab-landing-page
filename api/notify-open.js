...
module.exports = async function handler(req, res) {
  ...
  const isHarish     = program === 'harish-monthly' || /haris/i.test(program);
  const isSundayHiit = /sunday\s*hiit/i.test(program);

  const bookingLink = isHarish     ? 'https://gripandgrab.com/#premium-training'
                    : isSundayHiit ? 'https://gripandgrab.com/sunday-hiit.html'
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
  <tr><td style="background:linear-gradient(135deg,#ff6b6b 0%,#f7d794 100%);padding:40px 30px;text-align:center;">
    ...
    <p style="margin:10px 0 0;display:inline-block;background:rgba(0,0,0,0.12);padding:6px 16px;border-radius:20px;font-size:13px;font-weight:700;color:#000;letter-spacing:0.3px;">🔥 Train with Haristhenics — Spots just opened</p>
  </td></tr>
    ...
    try {
      const r = await fetch('https://api.resend.com/emails', {
        ...
        body:    JSON.stringify({
          ...
          to:      [lead.email],
          subject: isHarish
            ? `Slots open for Train with Haristhenics`
